...
function viewShopPage(i, itens, itensName)
    for inicio = i, #itensName, 6 do
        for j = inicio, math.min(inicio + 5, #itensName) do
            k = itensName[j]
            v = itens[k]
            print(j..") "..v.."x "..k)
        end
...
function viewShop()
   ...
   local select = 1
   local i = 1
   local qtdPages = math.ceil(#itensName / 6)
   local pageAtual = 1
   local result = nil
   ...
   while true do
       ...
       if qtdPages >= 1 and result ~= "nextPage" then
           viewShopPage(i, itens, itensName)
       elseif pageAtual < qtdPages and result == "nextPage" then
           i = i + 6
       ...
       print("\nTUTORIAL")
       print("- Se nao chegou tudo aguarde uns instante e escolha recarregar lista ou Proxima Lista se tiver 6 itens.")
       print("- Se tiver chegado tudo finalizar Compra.")
       
       result = moveMenu(menuCompra, select)
...
function optionsMenu(menu, select)
    local opcs = {
        ...
        Proximos_Itens = function() return "nextPage" end,
